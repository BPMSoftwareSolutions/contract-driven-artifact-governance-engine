...
function makeReleaseReceipt(
  ...
  const authorityClosureDisposition = closed
    ? inspected.authority.authorityClosure.admission
        .requiredDisposition
  ...
  return {
    ...
    authorityClosure: {
      authorityType:
        inspected.authority.authorityClosure.authorityType,
      profileSha256: sha256(
        canonicalJsonBytes(inspected.authority.authorityClosure)
      ),
    ...
    checks: [
      ...
      {
        checkId: "evaluate-release-authority-closure",
        disposition: authorityClosureDisposition
      },
      {
...
export function evaluateReleaseClaim(receipt, claim) {
  ...
  if (!policy) {
    ...
    for (const [field, expected] of [
      ...
      const observed =
        field === "authorityClosureDisposition"
          ? receipt?.authorityClosure?.disposition
          : receipt?.[field];
      if (observed !== expected) {
        findings.push({
          ...
          evidence: field,
          expected,
          observed: observed ?? "NOT_EVALUATED"
        });
      }
    }
  }
