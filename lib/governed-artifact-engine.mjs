...
export const DEFAULT_CONFORMANCE_PROFILE_PATH = path.join(
  packageRoot,
  "profiles",
  "closed-world-artifact-conformance.v1.json"
);
...
export const ENGINE_IDENTITY = "governed-artifact-engine.0.7.0";
...
function validateConformanceProfile(profile) {
  ...
  if (
    profile.profileType !== "governed-artifact-conformance-profile.v1" ||
...
export function migrateContract(options) {
  ...
  const sourceEdges = migrationRegistry.migrations.filter(
    (entry) => entry.sourceSchemaDigest === sourceEntry.digest
  );
  if (sourceEdges.length === 0) {
    const targetEdge = migrationRegistry.migrations.find(
      (entry) => entry.targetSchemaDigest === sourceEntry.digest
    );
    if (targetEdge) {
      const inspected = inspectContext({
        ...options,
