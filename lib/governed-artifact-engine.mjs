...
function projectedBytes(artifact) {
  ...
  return projector(artifact.projection.authority);
}
...
function validateCrossReferences(contract, projectorRegistry, verifierRegistry) {
  ...
  for (const artifact of contract.artifacts) {
    ...
    try {
      const bytes = projectedBytes(artifact);
...
export function projectArtifactFamily(options) {
  ...
  for (const artifact of contract.artifacts) {
    ...
    const expected = projectedBytes(artifact);
