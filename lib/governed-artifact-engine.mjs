...
export const DEFAULT_CONFORMANCE_PROFILE_PATH = path.join(
  packageRoot,
  "profiles",
  "closed-world-artifact-conformance.v3.json"
);
...
export const ENGINE_IDENTITY = "governed-artifact-engine.0.9.0";
...
function validateConformanceProfile(profile) {
  ...
  if (
    profile.profileType !== "governed-artifact-conformance-profile.v3" ||
  ...
  if (
    ...
    bodyPurity.profileType !== "semantic-execution-body.v1" ||
    bodyPurity.applicability !==
      "artifacts-declaring-projection-mappings" ||
    bodyPurity.authorityOperation !== "executeSemanticProjection" ||
...
function validateCrossReferences(contract, projectorRegistry, verifierRegistry) {
  ...
  for (const artifact of contract.artifacts) {
    ...
    if (
      ...
      contract.contract.contractVersion === "1.8.0" &&
      artifact.sourceAuthority?.projectionMappings.length > 0 &&
      !isSemanticExecutionBody(artifact)
    ) {
      findings.push({
    ...
    if (artifact.sourceAuthority) {
      ...
      if (isSemanticExecutionBody(artifact)) {
        const executionEdge =
          artifact.sourceAuthority.semanticEdges.find(
            (edge) =>
              edge.operation === "executeSemanticProjection"
          );
        ...
        const schemaArtifact = contract.artifacts.find(
          (entry) =>
            entry.artifactId ===
            semanticAuthority?.input?.schemaArtifactId
        );
        ...
        for (const [collection, identityField, authorityId] of [
          [
            artifact.sourceAuthority.failurePolicies,
            "failurePolicyId",
            semanticAuthority?.failure?.failurePolicyId
          ],
          [
            artifact.sourceAuthority.projectionMappings,
            "projectionMappingId",
            semanticAuthority?.projection?.projectionMappingId
          ],
          [
            artifact.sourceAuthority.resultContracts,
            "resultContractId",
            semanticAuthority?.result?.resultContractId
          ]
        ]) {
          if (
            collection.length !== 1 ||
        ...
        if (
          ...
          semanticAuthority?.authorityType !==
        ...
        const mapping =
          artifact.sourceAuthority.projectionMappings[0];
        ...
        if (
          JSON.stringify(canonicalize(semanticAuthority)) !==
...
function isSemanticExecutionBody(artifact) {
  return artifact.sourceAuthority?.semanticEdges.some(
    (edge) => edge.operation === "executeSemanticProjection"
  );
}

function verifySemanticExecutionBody(artifact, observation) {
  if (!isSemanticExecutionBody(artifact)) {
    return [];
  }
  const findings = [];
  const executionEdge = artifact.sourceAuthority.semanticEdges.find(
    (edge) => edge.operation === "executeSemanticProjection"
  );
...
function verifySourceAuthorityClosure(contract, artifact, observation) {
  ...
  const semanticExecutionBody = isSemanticExecutionBody(artifact);
  const semanticExecutionEdge =
    artifact.sourceAuthority.semanticEdges.find(
      (edge) => edge.operation === "executeSemanticProjection"
    );
  ...
  findings.push(
    ...verifySemanticExecutionBody(artifact, observation)
  );
  findings.push(
