...
export const DEFAULT_CONFORMANCE_PROFILE_PATH = path.join(
  packageRoot,
  "profiles",
  "closed-world-artifact-conformance.v8.json"
);
...
export const ENGINE_IDENTITY = "governed-artifact-engine.0.20.0";
...
function validateConformanceProfile(profile) {
  ...
  if (
    profile.profileType !== "governed-artifact-conformance-profile.v8" ||
...
function invocationAt(tokens, openParenIndex) {
  ...
  if (
    !isIdentifierToken(previous) &&
    !(
      isPropertyNameToken(previous) &&
      tokens[openParenIndex - 2]?.kind === "DotToken"
    )
  ) {
    return null;
  }
  ...
  while (
    ...
    isPropertyNameToken(tokens[cursor - 1])
  ) {
  ...
  if (
    tokens[cursor]?.kind === "DotToken" &&
    tokens[cursor - 1]?.kind === "CloseParenToken"
  ) {
    const nestedOpenParenIndex = findMatchingTokenBackward(
      tokens,
      cursor - 1,
      "OpenParenToken",
      "CloseParenToken"
    );
    const nestedInvocation = invocationAt(tokens, nestedOpenParenIndex);
    if (nestedInvocation) {
      return `${nestedInvocation}.${segments.join(".")}`;
    }
  }
...
function sourceParameterBindingRanges(tokens, functionRanges) {
  const ranges = [];
  const admit = (openParenIndex, closeParenIndex, bodyStart, endIndex) => {
    const names = new Set();
    for (
    ...
    if (names.size > 0) {
      ranges.push({ bodyStart, endIndex, names });
    }
  };

  for (const range of functionRanges) {
    if (range.functionKind === "arrow-function") {
      continue;
    }
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    if (tokens[index].kind !== "EqualsGreaterThanToken") {
      continue;
    }
    let openParenIndex;
    let closeParenIndex;
    if (isIdentifierToken(tokens[index - 1])) {
      openParenIndex = index - 2;
      closeParenIndex = index;
    } else if (tokens[index - 1]?.kind === "CloseParenToken") {
      closeParenIndex = index - 1;
      openParenIndex = findMatchingTokenBackward(
        tokens,
        closeParenIndex,
        "OpenParenToken",
        "CloseParenToken"
    ...
    const endIndex =
      tokens[bodyStart]?.kind === "OpenBraceToken"
        ...
        : tokens.length - 1;
    admit(openParenIndex, closeParenIndex, bodyStart, endIndex);
  }
  return ranges;
}
...
function dependencyInvocationOrigins(tokens, observedImports) {
  ...
  // Immutable bindings initialized from an imported call retain the imported
  // capability's provenance. Repeating the pass admits chains such as
  ...
  while (changed) {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
      if (
        tokens[index].kind !== "ConstKeyword" ||
        !isIdentifierToken(tokens[index + 1]) ||
...
function observeDependencyOperations(
  ...
  for (const invocation of invocationOccurrences) {
    const resolved = resolveOperation(invocation.invocation);
    if (!resolved) {
      continue;
    }
    ...
    observation.occurrences += invocation.occurrences;
...
function usesCompleteInvocationClosure(contract) {
  ...
  return (
    ...
    "closed-world-artifact-conformance.v8"
  );
}
...
function verifySourceAuthorityClosure(contract, artifact, observation) {
  const dependencyOperations = usesCompleteInvocationClosure(contract)
